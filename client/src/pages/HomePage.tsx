...
import { StatsBar } from '../component/ui/StatsBar';
import { useJobs } from '../features/jobs/api/useJobs';
import { useCategories } from '../features/jobs/api/useCategory'; // 🟢 Imported hook
import { JobCard } from '../features/jobs/components/JobCard';
import { CategoryGrid } from '../features/jobs/components/CategoryGrid'; // 🟢 Imported new grid component
...
export const HomePage = () => {
  ...
  const { data: categories, isLoading: catsLoading, isError: catsError } = useCategories(); // 🟢 Fetch categories
  ...
  // 🟢 Filter jobsList if a block category is currently selected
  const filteredJobs = selectedCategory 
    ? jobsList.filter((job: any) => job.categorySlug === selectedCategory || job.category?.slug === selectedCategory)
    : jobsList;

  if (jobsLoading || catsLoading) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="animate-pulse text-sm font-black uppercase tracking-widest text-slate-400">
        Loading Pipeline...
      </div>
  ...
  return (
    <main className="bg-[#F8FAFC]">
      ...
      {/* 🟢 NEW FEATURE: Category Grid inserted right below StatsBar */}
      <CategoryGrid 
      ...
      {/* Adjusted the margin top slightly here to space out nicely below the blocks */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
            <div className="flex justify-between items-end mb-10">
              <div>
                <h2 className="text-3xl font-black text-aventon-dark uppercase tracking-tighter">
                  {selectedCategory ? 'Filtered Openings' : 'Latest Openings'}
                </h2>
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-2">
                  Fresh opportunities updated daily
                </p>
            ...
            <div className="flex flex-col gap-6">
              {filteredJobs.length > 0 ? (
                ...
                <div className="p-12 border-2 border-dashed border-slate-200 rounded-[32px] text-center text-slate-400 font-bold uppercase text-[10px] tracking-widest">
                  No open postings in this specialization yet
                </div>
              )}
            </div>
          </div>
